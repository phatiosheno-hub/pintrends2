...
- 🖱️ **Klik gambar = monetisasi**: membuka link iklan direct (tab baru) + popup berisi tombol **Download Gambar** dan **Lanjut ke Pin Aslinya**
...
components/
├── PinGrid.jsx            ← Grid masonry + popup (Download / Lanjut) + link iklan
...
- **Link iklan direct** (dibuka saat gambar diklik) → ubah konstanta `AD_URL` di `components/PinGrid.jsx`. Mau hanya sekali per sesi? Bungkus `window.open` dengan guard `sessionStorage`.
