...
// lalu di tab ini muncul popup (modal) berisi Download Gambar + Lanjut ke Pin Aslinya.
const AD_URL =
      ...
      {selected && (
        <div className="modal-overlay" onClick={() => setSelected(null)}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={selected.title || 'Pratinjau gambar'}
          ...
          >
            ...
            <div className="modal-body">
              ...
              <img
                src={selected.url}
                alt={selected.title || `Gambar ${topic}`}
                referrerPolicy="no-referrer"
                className="modal-img"
              />
              {selected.title && <h3 className="modal-title">{selected.title}</h3>}
              <div className="modal-actions">
                <a
                  className="btn btn-primary"
                  href={`/api/download?url=${encodeURIComponent(
                    selected.url,
                  )}&name=${encodeURIComponent(`pintrend-${topic}-${selected.id}.jpg`)}`}
                >
                  ⬇ Download Gambar
                </a>
                <a
                  className="btn"
                  href={`https://www.pinterest.com/pin/${selected.id}/`}
                  target="_blank"
                  rel="noreferrer"
                >
                  ↗ Lanjut ke Pin Aslinya
                </a>
              </div>
              <p className="modal-note">Sumber gambar: Pinterest · klik “Download Gambar” untuk menyimpan ke perangkat</p>
            </div>
          </div>
        </div>
      )}
